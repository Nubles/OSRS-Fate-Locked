...
export const ChunkInfoBodyState: React.FC<Props> = ({ kind, onRetry }) => {
  if (kind === 'loading') {
    return (
  ...
  return (
    <div role={empty ? undefined : "alert"} aria-live={empty ? undefined : "assertive"} className="mt-2.5 flex items-start gap-2 rounded-lg border border-white/10 bg-white/[0.025] p-3 text-gray-400">
      ...
      <div>
        ...
        {!empty && onRetry && (
          <button type="button" onClick={onRetry} className="mt-2 text-[10px] font-semibold text-cyan-300 hover:text-cyan-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400">
            Retry loading chunk content
          </button>
        )}
      </div>
    </div>
