...
export function OnlineSyncDriver() {
  const { unlocks, runId, runRevision, keys, specialKeys, chaosKeys, fatePoints, activeBuff, pinnedGoals, linkedAccount, gameModeId } = useGame() as any;
  ...
  useEffect(() => {
    ...
    timer.current = window.setTimeout(() => {
      buildBundlePayload(unlocks, { runId, runRevision, keys, specialKeys, chaosKeys, fatePoints, activeBuff, pinnedGoals, linkedAccount, gameModeId: gameModeId ?? 'vanilla' })
        .then(({ compressed }) => relaySync.push(compressed))
        .catch(() => { /* surfaced via relaySync.status */ });
    }, 1500);
    return () => { if (timer.current) window.clearTimeout(timer.current); };
  }, [enabled, unlocks, runId, runRevision, keys, specialKeys, chaosKeys, fatePoints, activeBuff, pinnedGoals, linkedAccount, gameModeId]);
