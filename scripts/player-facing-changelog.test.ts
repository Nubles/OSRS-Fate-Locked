...
describe('quest and chunk audit release contracts', () => {
  ...
  it('runs quest verification once in the offline aggregate release order', () => {
    ...
    expect(contentVerify).toBe(
      'npm run diary:verify && npm run chunks:verify && npm run quests:verify && npm run quest-routes:verify && npm run walkthroughs:verify && npm run runeproof:catalogue:verify && npm run runeproof:coverage:verify && vitest run data/contentBaseline.test.ts data/tasksConsistency.test.ts utils/taskIdMigrations.test.ts utils/caProgress.test.ts',
    );
