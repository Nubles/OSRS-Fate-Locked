...
const LITE_OUT = resolve(ROOT, 'data', 'chunkContentLite.ts');
const SOURCE_URL =
...
function main(data) {
  ...
  // Slim per-chunk "what's here" for the RuneLite bundle export. Keyed "cx,cy"
  // (region id = cx<<8 | cy); categories: monsters / shops / farm patches / POIs.
  ...
  const flatNames = (a) => (a || []).map((x) => (Array.isArray(x) ? x[0] : x)).filter(Boolean);
  const capUniq = (a, n) => [...new Set(a)].slice(0, n);
  const lite = {};
  for (const [id, c] of Object.entries(doc.chunks || {})) {
    const rid = +id;
    if (!Number.isFinite(rid)) continue;
    const cx = rid >> 8, cy = rid & 255;
    const mon = capUniq([...(c.m || [])].sort((a, b) => (b[1] || 0) - (a[1] || 0)).map((p) => p[0]).filter(Boolean), 6);
    const shop = capUniq(flatNames(c.s), 8);
    const obj = flatNames(c.o);
    const farm = capUniq(obj.filter((n) => PATCH_RE.test(n)), 8);
    const poi = capUniq(obj.filter((n) => POI_RE.test(n) && !PATCH_RE.test(n)), 8);
    const e = {};
    if (mon.length) e.mon = mon;
    if (shop.length) e.shop = shop;
    if (farm.length) e.farm = farm;
    if (poi.length) e.poi = poi;
    if (Object.keys(e).length) lite[`${cx},${cy}`] = e;
  }
  const liteBody =
  ...
  console.log(`wrote data/chunkContentLite.ts — ${Object.keys(lite).length} chunks, ~${Math.round(liteBody.length / 1024)} KB`);
}
