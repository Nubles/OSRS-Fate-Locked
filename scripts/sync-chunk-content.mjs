...
function main(data) {
  ...
  // Slim per-chunk "what's here" (monster names) for the RuneLite bundle export.
  // Keyed "cx,cy" (canonical OSRS chunk coords; region id = cx<<8 | cy).
  const lite = {};
  for (const [id, c] of Object.entries(doc.chunks || {})) {
    ...
    const m = (c && c.m) || [];
    const names = [...m].sort((a, b) => (b[1] || 0) - (a[1] || 0)).map((p) => p[0]).filter(Boolean);
    const uniq = [...new Set(names)].slice(0, 6);
    if (uniq.length) lite[`${cx},${cy}`] = uniq;
  }
  const liteBody =
    `// AUTO-GENERATED by scripts/sync-chunk-content.mjs — do not edit by hand.\n` +
    `// Slim per-chunk "what's here" summary (monster names) for the RuneLite\n` +
    `// bundle export. Keyed by "cx,cy" (canonical OSRS chunk coords).\n\n` +
    `export const CHUNK_CONTENT_LITE: Record<string, string[]> = ${JSON.stringify(lite)};\n`;
