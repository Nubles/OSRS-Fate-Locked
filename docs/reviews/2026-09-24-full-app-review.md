...
which were verified in the browser instead. Fixing them turned up 14 more
issues, three of them risks the first report noted; they are fixed too (see
[Found while fixing](#found-while-fixing)).
Two follow-ups sit outside this change and are queued as separate tasks (see
...
- **G7:** kept interior sources rank after existing routes on a tie, so no public guide's top route or step changed. It does surface two interiors whose entry requirements were never recorded (see below).
...
- **Two interiors lack entry requirements.** Keeping interior sources (**G7**) shows that the Warriors' Guild basement (interior 11675) and Misthalin Manor record no entry requirement, so their sources read as free to reach; they rank after existing routes. The fix is wiki-verified entries in `data/sources/interior-access.json` and a regenerated `public/chunk-content.json`, which needs wiki access. It is queued as a separate task.
- **The plugin's inflate cap.** The overlay now refuses payloads that inflate past 8 MiB, but the RuneLite plugin inflates relay payloads without a limit. That change belongs in the plugin repository and is queued as a separate task.
